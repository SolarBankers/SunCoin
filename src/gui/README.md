...
Wallet apis service port is `7620`.
...
```bash
curl http://127.0.0.1:7620/wallet/newSeed
```
...
```bash
curl -X POST http://127.0.0.1:7620/wallet/create
```
...
```bash
curl -X POST http://127.0.0.1:7620/wallet/newAddress?id=2017_05_09_d554.wlt
```
...
```bash
curl http://127.0.0.1:7620/wallet/balance?id=2017_05_09_d554.wlt
```
...
```bash
curl -X POST \
  'http://127.0.0.1:7620/wallet/spend?id=2017_05_09_ea42.wlt&dst=2iVtHS5ye99Km5PonsB42No3pQRGEURmxyc&coins=1000000'
```
...
```bash
curl http://127.0.0.1:7620/balance\?addrs\=7cpQ7t3PZZXvjTst8G7Uvs7XH4LeM8fBPD,nu7eSpT6hr5P21uzw7bnbxm83B6ywSjHdq
```
...
```bash
curl http://127.0.0.1:7620/pendingTxs
```
...
```bash
curl http://127.0.0.1:7620/transaction?txid=a6446654829a4a844add9f181949d12f8291fdd2c0fcb22200361e90e814e2d3
```
...
```bash
curl http://127.0.0.1:7620/rawtx?txid=a6446654829a4a844add9f181949d12f8291fdd2c0fcb22200361e90e814e2d3
```
...
```bash
curl -X POST http://127.0.0.1:7620/injectTransaction -H 'content-type: application/json' -d '{
